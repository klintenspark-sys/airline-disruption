    ...
    with st.spinner("Agents are analyzing the disruption..."):
        ...
        try:
            r=httpx.post(
                ...
                json={"flight_id":selected},
                timeout=httpx.Timeout(600.0)
            )
            r.raise_for_status()
            result=r.json()
            st.session_state.ops_result=result
            st.session_state.ops_elapsed=round(time.time()-started,1)

        except httpx.TimeoutException:
            st.error(
                "AI analysis is taking longer than expected. "
                "The Neuro SAN multi-agent workflow exceeded the UI timeout."
            )
            ...
            st.error(
                "Cannot connect to the API bridge on port 8001. "
